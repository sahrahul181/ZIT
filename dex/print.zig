...
pub fn printDex(writer: anytype, dex: dexmod.DexFile, allocator: std.mem.Allocator) !void {
        ...
        for (class.methods.items) |method| {
            ...
            defer {
                for (insts) |inst| switch (inst) {
                    ...
                    .filled_new_array => |v| allocator.free(v.args),
                    else => {},
                };
...
pub fn printInstruction(writer: anytype, inst: Instruction) !void {
    switch (inst) {
        ...
        .filled_new_array => |v| try writer.print("filled-new-array {{{d} args}}, type@{d}", .{ v.args.len, v.type_idx }),
        .fill_array_data => |v| try writer.print("fill-array-data v{d}, +{d}", .{ v.array, v.payload_offset }),

        .throw_ => |v| try writer.print("throw v{d}", .{v.src}),

        .goto_ => |v| try writer.print("goto {d}", .{v.offset}),
        .packed_switch => |v| try writer.print("packed-switch v{d}, +{d}", .{ v.src, v.payload_offset }),
        .sparse_switch => |v| try writer.print("sparse-switch v{d}, +{d}", .{ v.src, v.payload_offset }),
