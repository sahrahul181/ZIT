...
pub const Interpreter = struct {
    ...
    pub fn invoke(self: *Interpreter, method: *MethodData, args: []const u64) InterpError!Value {
        ...
        if (method.jitEntry()) |jit_fn_ptr| {
            ...
            if (method.signature[0] == 'I' and method.signature.len == 2) {
                ...
                const func = @as(JITFn, @ptrFromInt(jit_fn_ptr));
                const res = func(@as(i64, @bitCast(args[0])), 0, 0, 0);
                return .{ .int = @as(i32, @truncate(res)) };
            }
        }
            ...
            switch (inst) {
                ...
                .new_instance => |v| {
                    const type_name = if (v.type_idx < self.dex.type_names.len)
                        self.dex.type_names[v.type_idx] else return InterpError.OutOfMemory;
                ...
                .new_array => |v| {
                    ...
                    @as(*i32, @alignCast(@ptrCast(arr))).* = size;
                ...
                .aget, .aget_boolean, .aget_byte, .aget_char, .aget_short => |v| {
                    ...
                    const elem_ptr = arr + 4 + @as(usize, @intCast(idx)) * 4;
                ...
                .aget_wide, .aget_object => |v| {
                    ...
                    const len = @as(*const i32, @ptrCast(@alignCast(@as(*anyopaque, @ptrFromInt(arr))))).*;
                    if (idx < 0 or idx >= len) return InterpError.ArrayIndexOutOfBounds;
                    const elem_ptr = arr + 4 + @as(usize, @intCast(idx)) * 8;
                ...
                .iget, .iget_boolean, .iget_byte, .iget_char, .iget_short => |v| {
                    ...
                    const cd = self.registry.get(fi.class_name) orelse { frame.regs[v.dest_or_src] = 0; continue; };
                ...
                .iget_wide, .iget_object => |v| {
                    ...
                    if (obj == 0) return InterpError.NullPointerException;
                    const fi = if (v.field_idx < self.dex.field_items.len) self.dex.field_items[v.field_idx] else {
                        frame.regs[v.dest_or_src] = 0; continue;
                    };
                    const cd = self.registry.get(fi.class_name) orelse { frame.regs[v.dest_or_src] = 0; continue; };
                    const off = cd.fieldOffset(fi.field_name) orelse 0;
                ...
                .if_eq => |v| { if (frame.regs[v.src1] == frame.regs[v.src2]) frame.pc = @intCast(@as(i64, frame.pc) - 1 + v.offset); },
                .if_ne => |v| { if (frame.regs[v.src1] != frame.regs[v.src2]) frame.pc = @intCast(@as(i64, frame.pc) - 1 + v.offset); },
                .if_lt => |v| { if (@as(i32, @bitCast(@as(u32, @truncate(frame.regs[v.src1])))) <  @as(i32, @bitCast(@as(u32, @truncate(frame.regs[v.src2]))))) frame.pc = @intCast(@as(i64, frame.pc) - 1 + v.offset); },
                .if_ge => |v| { if (@as(i32, @bitCast(@as(u32, @truncate(frame.regs[v.src1])))) >= @as(i32, @bitCast(@as(u32, @truncate(frame.regs[v.src2]))))) frame.pc = @intCast(@as(i64, frame.pc) - 1 + v.offset); },
                .if_gt => |v| { if (@as(i32, @bitCast(@as(u32, @truncate(frame.regs[v.src1])))) >  @as(i32, @bitCast(@as(u32, @truncate(frame.regs[v.src2]))))) frame.pc = @intCast(@as(i64, frame.pc) - 1 + v.offset); },
                .if_le => |v| { if (@as(i32, @bitCast(@as(u32, @truncate(frame.regs[v.src1])))) <= @as(i32, @bitCast(@as(u32, @truncate(frame.regs[v.src2]))))) frame.pc = @intCast(@as(i64, frame.pc) - 1 + v.offset); },
                .if_eqz => |v| { if (frame.regs[v.src] == 0) frame.pc = @intCast(@as(i64, frame.pc) - 1 + v.offset); },
                .if_nez => |v| { if (frame.regs[v.src] != 0) frame.pc = @intCast(@as(i64, frame.pc) - 1 + v.offset); },
                .if_ltz => |v| { if (@as(i32, @bitCast(@as(u32, @truncate(frame.regs[v.src])))) <  0) frame.pc = @intCast(@as(i64, frame.pc) - 1 + v.offset); },
                .if_gez => |v| { if (@as(i32, @bitCast(@as(u32, @truncate(frame.regs[v.src])))) >= 0) frame.pc = @intCast(@as(i64, frame.pc) - 1 + v.offset); },
                .if_gtz => |v| { if (@as(i32, @bitCast(@as(u32, @truncate(frame.regs[v.src])))) >  0) frame.pc = @intCast(@as(i64, frame.pc) - 1 + v.offset); },
                .if_lez => |v| { if (@as(i32, @bitCast(@as(u32, @truncate(frame.regs[v.src])))) <= 0) frame.pc = @intCast(@as(i64, frame.pc) - 1 + v.offset); },
                ...
                // ── Long arithmetic ───────────────────────────────────────────
                .add_long => |v| { const r: i64 = @as(i64, @bitCast(frame.regs[v.src1])) +% @as(i64, @bitCast(frame.regs[v.src2])); frame.regs[v.dest] = @bitCast(r); },
                .sub_long => |v| { const r: i64 = @as(i64, @bitCast(frame.regs[v.src1])) -% @as(i64, @bitCast(frame.regs[v.src2])); frame.regs[v.dest] = @bitCast(r); },
                .mul_long => |v| { const r: i64 = @as(i64, @bitCast(frame.regs[v.src1])) *% @as(i64, @bitCast(frame.regs[v.src2])); frame.regs[v.dest] = @bitCast(r); },
                .div_long => |v| {
                ...
                .shl_long  => |v| { const sh: u6 = @truncate(frame.regs[v.src2]); frame.regs[v.dest] = @bitCast(@as(i64, @bitCast(frame.regs[v.src1])) << sh); },
                .shr_long  => |v| { const sh: u6 = @truncate(frame.regs[v.src2]); frame.regs[v.dest] = @bitCast(@as(i64, @bitCast(frame.regs[v.src1])) >> sh); },
                .ushr_long => |v| { const sh: u6 = @truncate(frame.regs[v.src2]); frame.regs[v.dest] = frame.regs[v.src1] >> sh; },

                // ── Float arithmetic ──────────────────────────────────────────
                .add_float => |v| { const r: f32 = f32Reg(frame.regs[v.src1]) + f32Reg(frame.regs[v.src2]); frame.regs[v.dest] = @as(u32, @bitCast(r)); },
                .sub_float => |v| { const r: f32 = f32Reg(frame.regs[v.src1]) - f32Reg(frame.regs[v.src2]); frame.regs[v.dest] = @as(u32, @bitCast(r)); },
                .mul_float => |v| { const r: f32 = f32Reg(frame.regs[v.src1]) * f32Reg(frame.regs[v.src2]); frame.regs[v.dest] = @as(u32, @bitCast(r)); },
                .div_float => |v| { const r: f32 = f32Reg(frame.regs[v.src1]) / f32Reg(frame.regs[v.src2]); frame.regs[v.dest] = @as(u32, @bitCast(r)); },
                .rem_float => |v| { const r: f32 = @mod(f32Reg(frame.regs[v.src1]), f32Reg(frame.regs[v.src2])); frame.regs[v.dest] = @as(u32, @bitCast(r)); },

                // ── Double arithmetic ─────────────────────────────────────────
                .add_double => |v| { const r: f64 = f64Reg(frame.regs[v.src1]) + f64Reg(frame.regs[v.src2]); frame.regs[v.dest] = @bitCast(r); },
                .sub_double => |v| { const r: f64 = f64Reg(frame.regs[v.src1]) - f64Reg(frame.regs[v.src2]); frame.regs[v.dest] = @bitCast(r); },
                .mul_double => |v| { const r: f64 = f64Reg(frame.regs[v.src1]) * f64Reg(frame.regs[v.src2]); frame.regs[v.dest] = @bitCast(r); },
                .div_double => |v| { const r: f64 = f64Reg(frame.regs[v.src1]) / f64Reg(frame.regs[v.src2]); frame.regs[v.dest] = @bitCast(r); },
                .rem_double => |v| { const r: f64 = @mod(f64Reg(frame.regs[v.src1]), f64Reg(frame.regs[v.src2])); frame.regs[v.dest] = @bitCast(r); },
                ...
                .shl_int_lit8  => |v| { const sh: u5 = @intCast(v.lit & 0x1F); frame.regs[v.dest] = @bitCast(@as(i64, @as(i32, @bitCast(@as(u32, @truncate(frame.regs[v.src])))) << sh)); },
                .shr_int_lit8  => |v| { const sh: u5 = @intCast(v.lit & 0x1F); frame.regs[v.dest] = @bitCast(@as(i64, @as(i32, @bitCast(@as(u32, @truncate(frame.regs[v.src])))) >> sh)); },
                .ushr_int_lit8 => |v| { const sh: u5 = @intCast(v.lit & 0x1F); frame.regs[v.dest] = @as(u32, @truncate(frame.regs[v.src])) >> sh; },

                .add_int_lit16, .rsub_int_lit16, .mul_int_lit16, .div_int_lit16,
                .rem_int_lit16, .and_int_lit16, .or_int_lit16, .xor_int_lit16 => |v| {
                    const lit: i64 = v.lit;
                ...
                // ── Unary / Conversions ───────────────────────────────────────
                .neg_int     => |v| { const a: i32 = @bitCast(@as(u32, @truncate(frame.regs[v.src]))); frame.regs[v.dest] = @bitCast(@as(i64, -a)); },
                .not_int     => |v| { const a: u32 = @truncate(frame.regs[v.src]); frame.regs[v.dest] = ~a; },
                .neg_long    => |v| { const a: i64 = @bitCast(frame.regs[v.src]); frame.regs[v.dest] = @bitCast(-a); },
                .not_long    => |v| frame.regs[v.dest] = ~frame.regs[v.src],
                .neg_float   => |v| { const a: f32 = f32Reg(frame.regs[v.src]); frame.regs[v.dest] = @as(u32, @bitCast(-a)); },
                .neg_double  => |v| { const a: f64 = f64Reg(frame.regs[v.src]); frame.regs[v.dest] = @bitCast(-a); },
                .int_to_long    => |v| { const a: i32 = @bitCast(@as(u32, @truncate(frame.regs[v.src]))); frame.regs[v.dest] = @bitCast(@as(i64, a)); },
                .int_to_float   => |v| { const a: i32 = @bitCast(@as(u32, @truncate(frame.regs[v.src]))); frame.regs[v.dest] = @as(u32, @bitCast(@as(f32, @floatFromInt(a)))); },
                .int_to_double  => |v| { const a: i32 = @bitCast(@as(u32, @truncate(frame.regs[v.src]))); frame.regs[v.dest] = @bitCast(@as(f64, @floatFromInt(a))); },
                .long_to_int    => |v| frame.regs[v.dest] = @as(u32, @truncate(frame.regs[v.src])),
                .long_to_float  => |v| { const a: i64 = @bitCast(frame.regs[v.src]); frame.regs[v.dest] = @as(u32, @bitCast(@as(f32, @floatFromInt(a)))); },
                .long_to_double => |v| { const a: i64 = @bitCast(frame.regs[v.src]); frame.regs[v.dest] = @bitCast(@as(f64, @floatFromInt(a))); },
                .float_to_int   => |v| { const a: f32 = f32Reg(frame.regs[v.src]); frame.regs[v.dest] = @bitCast(@as(i64, @intFromFloat(a))); },
                .float_to_long  => |v| { const a: f32 = f32Reg(frame.regs[v.src]); frame.regs[v.dest] = @bitCast(@as(i64, @intFromFloat(a))); },
                .float_to_double => |v| { const a: f32 = f32Reg(frame.regs[v.src]); frame.regs[v.dest] = @bitCast(@as(f64, a)); },
                .double_to_int  => |v| { const a: f64 = f64Reg(frame.regs[v.src]); frame.regs[v.dest] = @bitCast(@as(i64, @intFromFloat(a))); },
                .double_to_long => |v| { const a: f64 = f64Reg(frame.regs[v.src]); frame.regs[v.dest] = @bitCast(@as(i64, @intFromFloat(a))); },
                .double_to_float => |v| { const a: f64 = f64Reg(frame.regs[v.src]); frame.regs[v.dest] = @as(u32, @bitCast(@as(f32, @floatCast(a)))); },
                .int_to_byte    => |v| { const a: i32 = @bitCast(@as(u32, @truncate(frame.regs[v.src]))); frame.regs[v.dest] = @bitCast(@as(i64, @as(i8, @truncate(a)))); },
                .int_to_char    => |v| frame.regs[v.dest] = frame.regs[v.src] & 0xFFFF,
                .int_to_short   => |v| { const a: i32 = @bitCast(@as(u32, @truncate(frame.regs[v.src]))); frame.regs[v.dest] = @bitCast(@as(i64, @as(i16, @truncate(a)))); },
                ...
                .cmpl_float, .cmpg_float => |v| {
                    ...
                    const r: i32 = if (std.math.isNan(a) or std.math.isNan(b)) nan_val
                ...
                .throw_ => |v| {
                    ...
                    const handler_pc = findHandler(frame.method.tries, frame.pc - 1, 0xFFFF_FFFF);
                    if (handler_pc) |hpc| {
                        frame.pc = hpc;
                    } else {
                ...
                .filled_new_array => |v| {
                    const arr_size = v.args.len * 8 + 8;
                    const arr = runtime.gcAlloc(0, arr_size);
                    @as(*i32, @alignCast(@ptrCast(arr))).* = @intCast(v.args.len);
                    for (v.args, 0..) |reg, i| {
                        const ep = @intFromPtr(arr) + 4 + i * 8;
    ...
    fn dispatchInvoke(self: *Interpreter, frame: *Frame, inv: *const Invoke) InterpError!Value {
        ...
        if (method.jitEntry()) |entry| {
            const raw_res = callJitCode(entry, args);
            if (method.signature[0] == 'V') {
                return .void_val;
            } else if (method.signature[0] == 'Z') {
...
inline fn f32Reg(raw: u64) f32 { return @bitCast(@as(u32, @truncate(raw))); }
inline fn f64Reg(raw: u64) f64 { return @bitCast(raw); }
